  ...
  <div class="app-container">
    ...
    <main ref="inputCardRef" class="glass-card input-group">
      ...
      <div class="options-row" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
        <div class="selector-group">
          <label class="selector-label">社交场景</label>
          <div class="style-selector">
            <button 
        ...
        <div class="selector-group">
          <label class="selector-label">回复风格</label>
          <div class="style-selector">
            <button 
      ...
      <div class="selector-group">
        <div style="display: flex; justify-content: space-between; align-items: center;">
          ...
          <div style="display: flex; gap: 0.5rem;">
            ...
            <button class="text-link-btn" @click="showGuideModal = true">高情商社交聊天与避坑红线指南</button>
          </div>
        </div>
        <textarea 
          v-model="userInput" 
...
const appTitle = ref(appConfig.title || '网腾无限AI - 高情商聊天回复与社交解套专家');
...
const guideRules = [
  {
    "title": "接话抛钩子",
    "advice": "回答对方问题时，顺带附赠一个新的有趣小信息或反问，保持话题双向互动",
    "avoid": "切忌只回答“嗯”、“好的”、“对”等终结对话的单字"
  },
  {
    "title": "化解“在吗”",
    "advice": "用幽默轻松的话语询问具体事情，如“在的，正在随时待命，请指示”",
    "avoid": "切勿单发一个“在”然后长时间没有下文"
  },
  {
    "title": "幽默化解尴尬",
    "advice": "面对调侃或尴尬，学会适度自嘲，自嘲是最高高级的社交安全垫",
    "avoid": "避免因小事玻璃心大发雷霆或在公开群聊强行辩驳"
  }
];
...
const displayResultText = computed(() => {
  ...
  return result.value.replace(/\[LIAOTIAN_SCORES\][\s\S]*?\[\/LIAOTIAN_SCORES\]/g, '').trim();
});

const parseAiScores = (rawText: string) => {
  const match = rawText.match(/\[LIAOTIAN_SCORES\](.*?)\[\/LIAOTIAN_SCORES\]/);
...
const loadHistory = () => {
  try {
    const raw = localStorage.getItem('liaotian_history_records');
...
const saveHistory = () => {
  localStorage.setItem('liaotian_history_records', JSON.stringify(historyList.value));
};
